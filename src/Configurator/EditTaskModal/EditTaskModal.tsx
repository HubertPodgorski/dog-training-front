    ...
    <Modal open={open} onClose={() => onClose()} className={styles.modal}>
      <div className={styles.modalContent}>
        <IconButton
          ...
          className={classNames(styles.iconWrapper, styles.closeModalIcon)}
        >
        ...
        <Section name='Kolejność'>
          <CustomSelect
            ...
            options={dogs.map((_, index) => ({
              id: `${index + 1}`,
              name: `${index + 1}  ${getDogNamesByOrder(taskList, index + 1)}`,
            }))}
        ...
        <Section name='Kolumna'>
          <CustomSelect
            onChange={async (column) => await saveTaskColumn(column as Column)}
            options={[
              { id: 'left', name: 'Lewa' },
              { id: 'right', name: 'Prawa' },
            ]}
        ...
        <Section name='Zadania ludzi'>
          <PeopleTasks savePeopleTasks={savePeopleTasks} peopleTasks={peopleTasks} />
        </Section>
      </div>
    </Modal>
  )
